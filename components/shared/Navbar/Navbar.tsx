  ...
  return (
    <nav ref={navigationRef} className={`flex justify-between min-h-[5rem] items-center mr-10 ml-8 988:mx-0 988:pt-4 relative z-50 988:sticky 988:top-0 988:flex-col 988:items-start 988:bg-[#041833] transition-all duration-300`}>
      <div className='988:flex 988:justify-between 988:items-center 988:w-full'>
        <Link onClick={() => setIsOpen(false)} as='/' href='/' placeholder='Homepage'>
      ...
      <div onMouseLeave={hideUnderline} className={`${ isOpen ? '988:h-screen 988:p-6': '988:h-0'} 988:space-y-4 988:space-y-reverse 988:top-6 988:overflow-hidden transition-all duration-200 flex 988:justify-end justify-center items-center space-x-7 988:space-x-0 h-fit relative 988:flex-col-reverse 988:bg-[#041833] 988:w-full`}>
          <Link onClick={() => setIsOpen(false)} onMouseOver={(e) => setUnderline(e)} id='Navbar-AboutUs-link' as='/AboutUs' href='/AboutUs'>About Us</Link>
          <Link onClick={() => setIsOpen(false)} onMouseOver={(e) => setUnderline(e)} id='Navbar-Community-link' as='/Community' href='/Community'>Community</Link>
          <Link onClick={() => setIsOpen(false)} onMouseOver={(e) => setUnderline(e)} id='Navbar-GameControls-link' as='/GameControls' href='/GameControls'>Game Controls</Link>
          <Link onClick={() => setIsOpen(false)} onMouseOver={(e) => setUnderline(e)} id='Navbar-Game-link' as='/Game' href='/Game'>Let&apos;s Play</Link>
          {
            isLoggedIn ? <LoggedInData setIsLoggedIn={setIsLoggedIn} username={username} setIsOpen={setIsOpen} />: <Link onClick={() => setIsOpen(false)} onMouseOver={(e) => setUnderline(e)} className='text-blue-950 988:text-white relative pl-2 pr-2 988:px-0 hover:scale-110 transition-all duration-300' id='Navbar-GameControls-link' as='/Login' href='/Login'>Log In/Register<Image className='absolute -top-[0.55rem] left-0 h-11 max-h-[2.75rem] w-full -z-[1] 988:hidden' height={100} width={100} src='/images/Lego Brick Fill.png' alt='Lego Brick' /></Link>
          }
...
const LoggedInData = (props: LoggedInProps) => {
  ...
  return (
    <div className='relative'>
      ...
      <div ref={menuRef} className={`${isOptionsOpen ? "": "hidden"} absolute w-52 right-0 988:translate-x-1/2 top-7 bg-white space-y-1 rounded-md p-1 transition-all duration-300`}>
        <h1 className='font-bold text-2xl ml-2 text-blue-950'>{props.username}</h1>
        <hr className='my-1' />
        <Link as='/User' href='/User' onClick={() => {setIsOptionsOpen(false); props.setIsOpen(false)}} className='flex items-center w-full text-blue-950 hover:text-blue-900 hover:bg-gray-300 px-2 py-1 rounded-lg transition-all duration-300'><FaUserCircle className='relative top-0.5 mr-5' /><span className='mx-auto'>My Models</span></Link>
